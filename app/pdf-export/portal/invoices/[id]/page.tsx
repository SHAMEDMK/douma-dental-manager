...
/** Page dédiée à l'export PDF (PDFShift) — portail client. Pas de barre, pas de badge, pas d'en-tête app. */
                ...
                {invoice.order.items.map((it, index) => (
                  <tr key={it.id} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                    <td className="px-4 py-3 text-gray-900 print:px-2 print:py-1.5 print:leading-tight">
                      {getLineItemSku(it) !== '-' && <span className="font-mono text-gray-600 mr-1">{getLineItemSku(it)}</span>}
