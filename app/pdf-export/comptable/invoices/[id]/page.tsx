...
/** Page dédiée à l'export PDF (PDFShift) — rôle COMPTABLE. Pas de barre, pas de badge, pas d'en-tête app. */
export default async function PdfExportComptableInvoicePage({
  ...
  const invoiceNumber = invoice.invoiceNumber || `FAC-${invoice.id.slice(-8).toUpperCase()}`;
          ...
          <div className="space-y-8">
              ...
              <div>
                {companySettings?.logoUrl && (
                  <div className="mb-4 print:mb-2">
                    <img src={companySettings.logoUrl} alt={companySettings.name || 'Logo'} className="h-16 w-auto object-contain print:h-10" />
                  </div>
                )}
                ...
                {companySettings && (
                  <>
                    {companySettings.address && <p className="text-xs text-gray-500 mt-1 print:mt-0.5 print:leading-tight">{companySettings.address}</p>}
                    ...
                    {companySettings.ice && <p className="text-xs text-gray-500 mt-1 print:mt-0.5 print:leading-tight">ICE: {companySettings.ice}</p>}
            ...
            <div className="mt-8 grid grid-cols-1 md:grid-cols-2 gap-8 text-sm print:grid-cols-2 print:mt-4 print:gap-4">
              <div className="bg-gray-50 p-4 rounded-lg print:bg-transparent print:border print:border-gray-300 print:p-2 print:py-1.5">
                ...
                {invoice.order.user.email && <div className="text-gray-600 text-xs mt-1 print:mt-0.5 print:leading-tight">{invoice.order.user.email}</div>}
                {invoice.order.user.phone && <div className="text-gray-600 text-xs mt-1 print:mt-0.5 print:leading-tight">{invoice.order.user.phone}</div>}
                ...
                {invoice.order.user.ice && <div className="text-gray-600 text-xs mt-1 print:mt-0.5 print:leading-tight">ICE: {invoice.order.user.ice}</div>}
              </div>
              <div className="bg-gray-50 p-4 rounded-lg print:bg-transparent print:border print:border-gray-300 print:p-2 print:py-1.5">
                ...
                <div className="space-y-2 print:space-y-1">
                  <div className="print:leading-tight">
                    ...
                    <span className="font-semibold text-gray-900 print:text-sm print:leading-tight ml-2">{invoiceNumber}</span>
                  </div>
                  <div className="print:leading-tight">
            ...
            <div className="keep-together">
              ...
              <div className="mt-6 text-sm print-no-break border-t pt-4 print:mt-3 print:pt-2 print:border-t print:border-gray-300">
                ...
                <div className="text-gray-900 font-semibold mt-2 uppercase print:mt-1 print:text-xs print:leading-tight">{numberToWords(taxTotals.ttc)}</div>
              </div>
