...
| `BLOB_READ_WRITE_TOKEN` | Token du Blob Store (Storage → Blob) pour les images uploadées | Production |

**Images (logo, produits)** : Sur Vercel, créez un **Blob Store** (Storage → Create → Blob). Le token `BLOB_READ_WRITE_TOKEN` est généré automatiquement.
